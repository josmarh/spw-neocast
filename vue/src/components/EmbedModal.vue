...
<template>
  <TransitionRoot as="template" :show="openEm">
    <Dialog as="div" class="relative z-10" @close="openEm = false">
            ...
            <DialogPanel class="relative bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full">
              ...
              <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button type="button" 
                    ...
                    @click="openEm = false" ref="cancelButtonRef"
                >Cancel</button>
              </div>
            </DialogPanel>
          </TransitionChild>
...
// const openEm = ref(true)

const {content} = defineProps({
    content: Object,
    openEm: Boolean
})
// const emit = defineEmits(['openEmbed'])
</script>