<template>
    <div>
        <website-component>
            <notification />
            <div class="mb-5">
                <router-link :to="{name: 'WebPage', params: { str: model.urlPath }}"
                    target="_blank"
            ...
            <form @submit.prevent="updateWebsite">
                ...
                <div class="mt-5">
                    <button type="submit" 
                        ...
                        font-medium text-sm w-full sm:w-auto px-6 py-2.5 
...
const getWebsiteInfo = async () => {
    ...
    await store
        .dispatch('editWebsite', route.params.whash)
        .then((res) => {
...
const setWebsiteInfo = (data) => {
    model.value.id = data.id;
    model.value.channel = JSON.parse(data.channel);
    model.value.title = data.title;
    model.value.urlPath = data.url_path;
    model.value.logo = data.logo;
    model.value.logoUrl = data.logo; // for preview purpose
    model.value.favicon = data.favicon;
    model.value.faviconUrl = data.favicon;// for preview purpose
    model.value.header = data.header;
    model.value.pageLayout = data.page_layout;
    model.value.bgColor = data.bg_color;
    model.value.fontColor = data.font_color;
    model.value.autoplay = data.autoplay;
    model.value.volume = data.volume == 1 ? true : false;
    model.value.controls = data.controls == 1 ? true : false;
    model.value.contentTitle = data.content_title == 1 ? true : false;
    model.value.showShareBtn = data.share_button == 1 ? true : false;
    model.value.footer = data.footer_text;
    model.value.siteMeta = data.seo_site_meta;
}
...
const updateWebsite = async () => {
    ...
    await store
        .dispatch('updateWebsite', model['_rawValue'])
        .then((res) => {
            internalInstance.appContext.config.globalProperties.$Progress.decrease(40);
            internalInstance.appContext.config.globalProperties.$Progress.finish();
