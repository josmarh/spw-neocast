    ...
    <div>
        <page-component title="Uploads" class="min-h-screen">
            ...
            <div id="file-drag-drop">
                <form @submit.prevent="uploadFiles" enctype="multipart/form-data" id="fileform">
                    ...
                    <div v-for="vid in upload.files" :key="vid.name" class="flex justify-center items-center w-full mt-3">
                        <div id="toast-default" class="flex items-center justify-between w-full w-96 p-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800" role="alert">
                            <div class="flex">
                            ...
                            <button type="button" @click="removeFile(vid.name)" v-if="!upload.isUploading && !vid.uploaded"
                            ...
                            <!-- loader -->
                            <div role="status" v-else-if="upload.isUploading && !vid.uploaded">
                            ...
                            <!-- SVG tick uploaded -->
                            <div v-else-if="vid.uploaded" class="inline-flex">
...
const uploadFiles = () => {
    ...
    for(const [i, item] of upload.value.files.entries()) {
        store
            .dispatch('uploadFiles', item)
            .then(res => {
                store.state.uploadProgress = 0;
                upload.value.files[i].uploaded = true

                if(i+1 === upload.value.files.length) {
