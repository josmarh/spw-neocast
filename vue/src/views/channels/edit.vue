            ...
            <div class="py-6 px-10">
                <form @submit.prevent="updateChannel">
                                ...
                                <div class="grid xl:grid-cols-12 xl:gap-4">
                                    <div class="xl:col-span-1">
                                        ...
                                        <select
                                            ...
                                            id="select_schedule"
                                            class="block py-2 px-0 w-[5rem] text-sm text-gray-500
                                    ...
                                    <div class="grid xl:grid-cols-7 xl:gap-4">
                                        <label v-for="(day, index) in weekday" 
                                            ...
                                            <div class="flex items-center mb-4">
                                                ...
                                                :checked="selectedday.filter(e => e.day === day.day).length > 0 ? true : false">
                                            </div>
                                        </label>
                                    </div>
                                </div>
                        ...
                        >
                            <div class="grid xl:grid-cols-12 xl:gap-3">
                                ...
                                <div class="col-span-8">
                                    ...
                                    <div v-if="ChannelPlaylistCheck == 3">
                                        <div class="p-20 w-full bg-black border shadow-md sm:p-36 dark:bg-gray-800 dark:border-gray-700 text-center">
                                            <span class="text-white font-medium font-bold text-2xl capitalize">no video available</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <button type="submit" 
                        ...
                        dark:focus:ring-blue-800" :disabled="isDisabled">Accept
                    </button>
...
const dataPlacement = async (data) => {
    ...
    channelModel.value.description = data.description == null ? '' : data.description
    selectdaymodal.value.time = data.start_time
    selectedday.value = data.schedule_daytime ? JSON.parse(data.schedule_daytime) : [];

    if(data.schedule_daytime){
        for(let d of JSON.parse(data.schedule_daytime)) {
            let weekdayIndex = weekday['_rawValue'].findIndex((obj => obj.day == d.day));
            //Update object's name property.
            weekday['_rawValue'][weekdayIndex].time = d.starttime;
        }
    }
    ...
    await store
        ...
        .then((res) => {
            ...
            if(res.data.length) {
                ...
                streamLink.value = `${import.meta.env.VITE_STREAM_URI}/channels/${data.stream_name}.m3u8`
                if(data.channel_type.includes('Linear')) {
                    videoOptionsLinear.sources[0].src = `${import.meta.env.VITE_STREAM_URI}/channels/${data.stream_name}.m3u8`;
