<template>
    <div>
        <page-component-vue title="Permissions" :class="[ permitListCheck == 0 || permitListCheck == 1 || permissions.data.length < 3 ? 'h-screen bg-gray-100' : '' ]">
            <notification-vue />
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <div class="p-4">
                    <div class="inline-flex rounded-md shadow-sm mb-3" role="group">
                        <button type="button" @click="addPermitModal" 
                ...
                <div v-if="permitListCheck == 2">
                            ...
                            <tr v-for="item in permissions.data" :key="item.id"
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex">
                                        <!-- edit -->
                                        <div class="">
                                            <button type="button" @click="updatePermitModal(item)"
                                        ...
                                        <!-- delete -->
                                        <div class="">
                                            <button type="button" @click="deletePermitModal(item)"
                                                class="text-gray-500 bg-gray-100 hover:bg-gray-200 
                                                focus:outline-none focus:ring-gray-100 
                                                font-medium text-sm px-5 py-2.5 text-center 
                                                inline-flex items-center dark:focus:ring-gray-500 mr-2">
                    ...
                    <div class="flex justify-center mt-5 mb-5">
                        <nav class="relative z-0 inline-flex justify-center rounded-md shadow-sm"
                            ...
                            <a 
                                v-for="(link, i) of permissions.meta.links" 
                                :key="i"
                                :disabled="!link.url"
                                href="#"
                                ...
                                :class="[
                                    ...
                                    i === 0 ? 'rounded-l-md' : '',
                                    i === permissions.meta.links.length - 1 ? 'rounded-r-md' : '',
                                ]"
                                v-html="link.label">
                            </a>
                        </nav>
            ...
            <div v-if="permitListCheck == 1" class="flex justify-center py-20">
                <!-- spinner state -->
            ...
            <TransitionRoot as="template" :show="openCreatePermit">
                <Dialog as="div" class="relative z-10" @close="openCreatePermit = false">
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            <div class="sm:flex sm:items-start">
                                <div class="mt-3 text-center sm:mt-2 sm:ml-0 sm:text-left">
                                    <DialogTitle as="h3" class="text-lg leading-6 font-medium text-gray-900"> Add Permission </DialogTitle>
                                </div>
                            </div>
                            <div class="mt-6">
                                <form @submit.prevent="addPermit">
                                    ...
                                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                        <button type="button" 
                                            ...
                                            @click="openCreatePermit = false" ref="cancelButtonRef">Cancel
                                        </button>
                                        <button type="submit"
                                            class="mt-3 w-full inline-flex justify-center border 
            ...
            <!-- update role -->
            <TransitionRoot as="template" :show="openUpdatePermit">
                <Dialog as="div" class="relative z-10" @close="openUpdatePermit = false">
                <TransitionChild as="template" 
                    enter="ease-out duration-300" 
                    enter-from="opacity-0" 
                    enter-to="opacity-100" 
                ...
                <div class="fixed z-10 inset-0 overflow-y-auto">
                    ...
                    <TransitionChild as="template" 
                        ...
                        <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                            ...
                            <div class="mt-6">
                                <form @submit.prevent="updatePermit">
                                    ...
                                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                        <button type="button" 
                                            ...
                                            text-gray-700 hover:bg-gray-50  sm:mt-0 sm:ml-3 
                                            sm:w-auto sm:text-sm" 
                                            @click="openUpdatePermit = false" ref="cancelButtonRef">Cancel
                                        </button>
                                        <button type="submit"
                                            class="mt-3 w-full inline-flex justify-center border 
                                            shadow-sm px-6 py-2 bg-indigo-600 hover:bg-indigo-700
            ...
            <TransitionRoot as="template" :show="openDeletePermit">
                <Dialog as="div" class="relative z-10" @close="openDeletePermit = false">
                    <TransitionChild as="template" 
                        enter="ease-out duration-300" 
                        enter-from="opacity-0" 
                        enter-to="opacity-100" 
                            ...
                            >
                                <DialogPanel class="relative bg-white text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full">
                                    ...
                                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                                        ...
                                        @click="openDeletePermit = false" ref="cancelButtonRef">Cancel
                                        </button>
                                        <button type="button" 
                                            ...
                                            sm:w-auto sm:text-sm" 
                                            @click="deletePermit(model.id)" :disabled="isDisabled">Delete
                                        </button>
                                    </div>
                                </DialogPanel>
                            </TransitionChild>
...
const permissions = computed(() => store.state.userGuard);
const internalInstance = getCurrentInstance();
const openCreatePermit = ref(false);
const openUpdatePermit = ref(false);
const openDeletePermit = ref(false);
const isDisabled = ref(false);

let permitListCheck = ref(0)
let model = {
    id: '',
    name: '',
};
...
watch(permitName, (newVal, oldVal) => {
    setTimeout(() => {
        searchPermit();
    }, 2000);
});
...
const getPermits = () => {
    ...
    store
        ...
        .then((res) => {
            ...
            if(res.data.length)
                permitListCheck.value = 2;
            else
                permitListCheck.value = 3;
        })
        .catch((err) => {
...
const searchPermit = () => {
    ...
    permitListCheck.value = 1;
    store
        .dispatch('getUserPermissions', {url: `${import.meta.env.VITE_API_BASE_URI}/api/permissions?name=${permitName.value}`})
        .then((res) => {
            internalInstance.appContext.config.globalProperties.$Progress.decrease(40);
            internalInstance.appContext.config.globalProperties.$Progress.finish();
            if(res.data.length)
...
const addPermitModal = () => {
    model.name = '';
    openCreatePermit.value = true
}

const updatePermitModal = (role) => {
    model.id = role.id;
    model.name = role.name;
    openUpdatePermit.value = true;
}

const deletePermitModal = (role) => {
    model.id = role.id;
    model.name = role.name;
    openDeletePermit.value = true
}
...
const getForPage = (ev,link) => {
    ...
    store.dispatch('getUserPermissions', {url: link.url});
}
