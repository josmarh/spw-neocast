<template>
  <div>
    <page-component-vue title="Live Streams" :class="[dataCheck == 0 || dataCheck == 1 || content.data.length < 3 ? 'h-screen bg-gray-100' : '' ]">
      <notification-vue />
      <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
        <div class="p-4">
          <div class="inline-flex rounded-md shadow-sm mb-3" role="group">
            <button @click="_createLiveStream"
          ...
          <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
            ...
            <tbody>
              <tr v-for="item in content.data" :key="item.id">
                ...
                <td class="px-6 py-4">
                  <div v-if="!JSON.parse(item.channels).length"><em>No channels added</em></div>
                  <div v-for="c in JSON.parse(item.channels)" :key="c.id">
                    {{c.title}},
                  </div>
                </td>
      ...
      <div v-if="dataCheck == 3" >
        <div class="flex justify-center items-center">
          <div class="p-4 xl:w-[70rem] text-center sm:p-8 dark:bg-gray-800 dark:border-gray-700">
            <h5 class="text-2xl text-gray-900 dark:text-white">No live stream available</h5>
        ...
        <div class="justify-center items-center space-y-4 sm:flex sm:space-y-0 sm:space-x-4">
          <div class="p-4 max-w-[40rem] bg-white border shadow-md sm:p-8 dark:bg-gray-800 dark:border-gray-700">
            <div class="flex mb-4">
              <h5 class="text-xl font-bold leading-none text-gray-900 dark:text-white">Take advantage of the following features:</h5>
            </div>
            <div class="flow-root">
                  ...
                  <div class="flex  space-x-4">
                    <div class="">
                      <p class="text-sm text-gray-900 dark:text-white">
                        Broadcast live via any RTMP hardware or software encoder such as OBS, Wirecast, vMix.
                      </p>
                    </div>
                  </div>
                </li>
                ...
                <DialogPanel 
                  ...
                  <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                      ...
                      <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        ...
                        <div class="mt-2">
                          <p class="text-sm text-gray-500">Are you sure you want to delete this live stream? 
                  ...
                  <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="button" 
                      ...
                      sm:w-auto sm:text-sm" @click="_delete(model.id)">Delete
                    </button>
                    <button type="button" 
...
const content = computed(() => store.state.liveStream);
...
const _createLiveStream = () => {
  internalInstance.appContext.config.globalProperties.$Progress.start();
  store
    .dispatch('liveStreamCreate', liveModel['_rawValue'])
    .then((res) => {
      internalInstance.appContext.config.globalProperties.$Progress.finish();
      router.push({name: 'LiveStreamEdit', params: {hash: res.data.lhash}})
    })
    .catch((err) => {
...
const _filters = async (title) => {
  internalInstance.appContext.config.globalProperties.$Progress.start();
  dataCheck.value = 1;

  await store
    .dispatch('liveStreamFilter', title)
    .then((res) => {
      internalInstance.appContext.config.globalProperties.$Progress.decrease(40);
      internalInstance.appContext.config.globalProperties.$Progress.finish();
      if(res.data.length)
...
const _delete = async (id) => {
  ...
  await store
    .dispatch('liveStreamDelete', id)
    .then((res) => {
      store.dispatch('liveStreams')
      internalInstance.appContext.config.globalProperties.$Progress.decrease(40);
      internalInstance.appContext.config.globalProperties.$Progress.finish();
      openDelete.value = false;
...
const getForPage = (ev,link) => {
  ...
  store.dispatch("liveStreams", {url: link.url})
}
